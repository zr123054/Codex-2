...
本文档基于当前 FE25 Test APP 前端应用基线整理，供后端开发、联调和接口评审使用。当前仓库已经完成面向 iOS / Android 可安装应用的离线 UI、前端交互和状态流转；接入真实后端时可按本文档拆分接口、数据模型与任务状态。


## 0. 与 UI / 前端资料的关系

- `docs/UI_DESIGN_SPEC.md` 定义真实产品 APP 的视觉规范、页面结构、组件状态和交互验收点。
- `docs/BACKEND_UI_HANDOFF.md` 是 UI 工作完成后直接交给后端的页面状态、接口依赖和联调验收文件。
...
- Capacitor 移动端应用壳：加载本地 `index.html`，可生成 iOS / Android 原生工程。
...
- 通用 UI：移动端安全区适配、底部 Tab、Toast、弹窗、进度条、禁用态、危险操作二次确认。
